...
LeakyReLU::LeakyReLU(gpu::Device *dev, int inputChannel, int inputHeight,
                     int inputWidth)
  ...
  cl_int err = 0;
  kernel_ = clCreateKernel(dev_->ClDnnHandle()->program, "leakyReLU", &err);
  CheckCl(err);
  err = 0;
  kernel_img =
      clCreateKernel(dev_->ClDnnHandle()->program, "leakyReLU_img", &err);
  CheckCl(err);
}

LeakyReLU::~LeakyReLU() {}

void LeakyReLU::Forward(gpu::Memory *input, gpu::Memory *output) {
