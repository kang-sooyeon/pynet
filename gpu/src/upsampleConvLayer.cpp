...
UpsampleConvLayer::UpsampleConvLayer(gpu::Device *dev, int inputChannel,
                                     int inputHeight, int inputWidth,
                                     int outputChannel, int kernelSize,
                                     ConvolutionMode mode, int scale,
                                     int stride, bool reluOn)
  ...
  // upsample layer
  upsample_ =
      new Upsample(dev_, inputChannel_, inputHeight_, inputWidth_, scale_);
  upsample_->GetOutputSize(c, h, w);

  // reflectionpad
  reflectionPadding_ = kernelSize_ / 2;
  reflectionPad_ = new ReflectionPad2d(dev_, c, h, w, reflectionPadding_);
  reflectionPad_->GetOutputSize(c, h, w);

  // convolution2d
  conv2d_ = new Convolution2d(dev_, c, h, w, outputChannel, kernelSize_,
                              stride_, mode_);
  conv2d_->GetOutputSize(c, h, w);

  // leakyReLU
  if (reluOn_) {
    relu_ = new LeakyReLU(dev_, c, h, w);
...
void UpsampleConvLayer::Forward(gpu::Memory *input, gpu::Memory *output,
                                gpu::Memory *l1, gpu::Memory *l2) {
  upsample_->Forward(input, l1);
  reflectionPad_->Forward(l1, l2);
  conv2d_->Forward(l2, output);
...
UpsampleConvLayer2::UpsampleConvLayer2(gpu::Device *dev, int inputChannel,
                                       int inputHeight, int inputWidth,
                                       int outputChannel, int kernelSize,
                                       int inputN, ConvolutionMode mode,
  ...
  int h, w, c;
  // upsample layer
  upsample_ = new Upsample(dev, inputChannel_ / inputN_, inputHeight_,
                           inputWidth_, scale_);
  upsample_->GetOutputSize(c, h, w);
...
UpsampleConvLayer2::~UpsampleConvLayer2() {
  for (int i = 0; i < inputN_; i++) delete conv2d_[i];
  delete bias_;
}

void UpsampleConvLayer2::LoadWeight(std::ifstream &fp) {
  for (int i = 0; i < inputN_; i++) {
...
void UpsampleConvLayer2::Forward(vector<gpu::Memory *> inputs,
                                 gpu::Memory *output, gpu::Memory *l1,
                                 gpu::Memory *l2) {
  upsample_->Forward(inputs[0], l1);
  reflectionPad_->Forward(l1, l2);
  conv2d_[0]->Forward(l2, output);

  for (int i = 1; i < inputN_; i++) {
    upsample_->Forward(inputs[i], l1);
    reflectionPad_->Forward(l1, l2);
    conv2d_[i]->Forward(l2, l1);
...
void UpsampleConvLayer2::AddBias(gpu::Memory *bias, gpu::Memory *input) {
